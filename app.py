...
def chats():
    ...
    #    messages = db.execute("SELECT * FROM messages WHERE sender=? AND receiver=?", session["user_id"], request.args.get("friend"))
    #    return redirect("chat.html", friend1 = session["user_id"], friend2=request.args.get("friend"), messages=messages)
    #else:
...
def chat():
    if not request.args.get("friend"):
        ...
        messages = db.execute("SELECT * FROM messages WHERE (sender=? AND receiver=?) OR (sender=? AND receiver=?)", session["user_id"], request.args.get("friend"), request.args.get("friend"), session["user_id"])
        if request.form.get("message") and (len(messages)==0 or messages[len(messages)-1]["message"] != request.form.get("message")) :
            print(request.form.get("message"))
            db.execute("INSERT INTO messages(sender,receiver,message,date) VALUES(?,?,?,?)", session["user_id"], request.args.get("friend"), request.form.get("message"), time())
        ...
        return render_template("chat.html", friend1_username=username, friend2_username=friendname, friend1=session["user_id"], friend2=request.args.get("friend"), messages=messages)
